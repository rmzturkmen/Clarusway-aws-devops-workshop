...
def convert(millisecond):
    ...
    return f'{hours} hour/s '*(hours != 0) + f'{minutes} minute/s '*(minutes != 0) + f'{seconds} second/s '*(seconds != 0) \
        or f'just {millisecond} millisecond/s'
...
def main_post():
    ...
    return render_template('result.html', developer_name=' Ramazan', milliseconds=alpha, result=convert(int(alpha)))
